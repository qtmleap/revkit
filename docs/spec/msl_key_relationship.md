...
## 9. 解明済み: key 33.6 平文の詳細構造

### 352B バリアント (map(7), 標準)

| 領域 | バイト範囲 | サイズ | 内容 |
|------|-----------|--------|------|
| CBOR ヘッダ | [0:128] | 128B | **全セッション固定** — `d9d9f7a7` (tag 55799 + map(7)) + 2 CBOR pairs + opaque blob |
| セッション領域 | [128:288] | 160B | TFIT(DH PubKey) 128B + MGK Pair 32B — セッション単位で固定 |
| リクエスト固有 | [288:352] | 64B | 7B ランダム nonce ×3 (CBOR オーバーヘッド付き) + 4B テール |

- CBOR ヘッダは 165 サンプル中 **100% 同一** → Argo バイナリから静的コピー可能
- リクエスト固有領域の 7B nonce はタイムスタンプでもカウンタでもない **ランダム値**
- 同じ 7B が 3 回繰り返される (CBOR エンコードオーバーヘッドが各所で異なる)

### 144B バリアント (map(6), 軽量)

- map(7) の 1 エントリが欠落 (map(6))
- **セッション領域なし** — DH 公開鍵も MGK も含まない
- ヘッダ 89B + 可変領域 55B
- キャプチャ全体に散在 (17/239 = 7%) — セッション開始/終了とは無関係
- **DH 認証なしの軽量 appboot リクエスト** と推定
